...
internal class OpeningScene
{
    ...
    public void Play()
    {
        ...
        ShowLogo(); // 로고 강하 애니메이션

        ts[0].Wait(); // 음악이 끝날 때까지 대기
        _run = false; // 음악 끝나면 병아리도 정지 준비
        ts[1].Wait();
    ...
    private void ShowPressAnyKey()
    {
        ...
        while (!Console.KeyAvailable)
        {
            lock (_lk)
            {
                ...
                Console.Write("\x1b[38;2;255;255;255m" + msg + "\x1b[0m"); // 흰색 문구
            }
            Thread.Sleep(500);
            lock (_lk)
            {
                Console.SetCursorPosition(mx, my);
                Console.Write(new string(' ', msg.Length)); // 깜빡임 효과
            }
            Thread.Sleep(500);
        }
        Console.ReadKey(true); // 입력된 키 소모
    }
